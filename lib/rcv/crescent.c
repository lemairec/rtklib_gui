...
static int decode_cresraw2(raw_t *raw)
{
    ...
    for (i=n=0,p+=16;i<15&&n<MAXOBS;i++) {
        ...
        if      (cp[0]-pr[0]/lam[0]<-4096.0) cp[0]+=8192.0;
        else if (cp[0]-pr[0]/lam[0]> 4096.0) cp[0]-=8192.0;
        ...
        if (i<12) {
            ...
            if (pr[1]!=0.0) {
                ...
                if      (cp[1]-pr[1]/lam[1]<-4096.0) cp[1]+=8192.0;
                else if (cp[1]-pr[1]/lam[1]> 4096.0) cp[1]-=8192.0;
            }
            else cp[1]=0.0;
        }
        raw->obs.data[n].time=time;
